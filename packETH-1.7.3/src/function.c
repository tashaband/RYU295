...
int send_packet(GtkButton *button, gpointer user_data)
{
	...
	GtkWidget *en1, *en2, *en3, *en4, *ckbt1, *ckbt2, *ckbt3, *ckbt4, *ckbt5, *ckbt6, *xoptm, *yoptm, *xmenu_item, *ymenu_item; 
	...
	/* is it the generator that sends different sequences? */
		...
		GtkWidget *optm11, *optm111, *optm22;
...
int ipv6_get(GtkButton *button, gpointer user_data) {
	...
	GtkWidget *src6ip, *dst6ip, *src6bt, *dst6bt, *payloadlength_bt, *pay_text_e;
...
int icmpv6_get(GtkButton *button, gpointer user_data, guint32 pseudo_header_sum) {
        ...
        int icmp_start, icmp_stop, checksum_start, payload_length, i, j;
	int cks_start, cks_stop, odd;
        guint32 icmpcksum;
	...
	/* optional message body */
        if ( (strlen(msgbody_t) != 0) && (strlen(msgbody_t) != 8)) {
                error("Error: Wrong length of icmpv6 message body");
                return -1;
        }
	...
	/* data */
        if (GTK_TOGGLE_BUTTON(data_bt)->active) {
         ...
         if (checksum_start > 0) {
                ...
                icmpcksum = (guint32)(cks_stop - cks_start);
                ...
                icmpcksum = icmpcksum + get_checksum32(cks_start, cks_stop+odd);
                while (icmpcksum >> 16)
                        icmpcksum = (icmpcksum & 0xFFFF)+ (icmpcksum >> 16);
                /* the one's complement */
                icmpcksum = (-1) - icmpcksum;

                // -58 stands for 3a what is protocil number for icmpv6
                if (ipv6_used == 1)
